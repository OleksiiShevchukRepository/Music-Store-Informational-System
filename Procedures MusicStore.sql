...
CREATE PROCEDURE spAlbumsInMusicStore_Filters
	@GenreId INT --review O.S: it's necessary to validate input arguments
AS
BEGIN
...
CREATE PROCEDURE spCreateSoldItem
	@AlbumId INT,
	@Amount INT,
	@Price DECIMAL(19, 2)--review O.S: it's necessary to validate input arguments
AS
BEGIN
	DECLARE @CheckID INT;
	DECLARE	@CheckDate DATETIME;
	SET @CheckID = (SELECT TOP 1 Id FROM tblCheck ORDER BY Id DESC);--review O.S: it's better to use Scope_Identity()
